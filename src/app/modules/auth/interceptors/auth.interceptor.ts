...
const setBearToken: <T>(request: HttpRequest<T>) => HttpRequest<T> = <T>(request: HttpRequest<T>): HttpRequest<T> => {
  let token: string | undefined = UserStorageUtils.getToken(TypeToken.ACCESS_TOKEN);
...
const handle401Error: <T>(request: HttpRequest<T>, next: HttpHandlerFn, authService: AuthService) => Observable<HttpEvent<unknown>> = <T>(
  ...
  .pipe(
    switchMap((response: JwtTokens): Observable<HttpEvent<unknown>> => {
      ...
      if (!currentAuthUser) {
        return throwError((): HttpErrorResponse => new HttpErrorResponse({
          status: 401,
