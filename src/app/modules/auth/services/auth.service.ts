...
export class AuthService {
  ...
  signUp(signUpDto: SignUp): void {
    this.httpClient.post<void>(`${LOCAL_API_PATH}${API_URI_CONF.auth.signUp()}`, signUpDto).subscribe({
      next: (): void => {
        this.router.navigate([`/${AppUrlConf.signIn}`]);
      },
      error: (error): void => {
        console.log(error);
      }
  ...
  signIn(signInDto: SignIn): void {
    this.httpClient.post<AuthUser>(`${LOCAL_API_PATH}${API_URI_CONF.auth.signIn()}`, signInDto).subscribe({
  ...
  refreshTokens(): Observable<JwtTokens> {
    return this.httpClient.post<JwtTokens>(`${LOCAL_API_PATH}${API_URI_CONF.auth.refreshToken()}`, {});
  }
